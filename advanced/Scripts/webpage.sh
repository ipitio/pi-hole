...
UpdateSpeedTest() {
    ...
    tmux send-keys -t pimod "curl -sSL https://github.com/arevindh/pihole-speedtest/raw/master/mod.sh | tac | tac | bash -s -- up ${args[2]}" Enter
}
...
UninstallSpeedTest() {
    ...
    tmux new-session -d -s pimod
    tmux send-keys -t pimod "curl -sSL https://github.com/arevindh/pihole-speedtest/raw/master/mod.sh | tac | tac | bash -s -- un" Enter
}
