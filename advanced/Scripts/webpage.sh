...
UpdateSpeedTest() {
    ...
    tmux send-keys -t pimod "curl -sSL https://github.com/ipitio/pihole-speedtest/raw/ipitio/mod.sh | bash -s -- up ${args[2]}" Enter
}
...
UninstallSpeedTest() {
    ...
    tmux kill-session -t pimod &> /dev/null
    tmux new-session -d -s pimod
    tmux send-keys -t pimod "curl -sSL https://github.com/ipitio/pihole-speedtest/raw/ipitio/mod.sh | bash -s -- un" Enter
}
