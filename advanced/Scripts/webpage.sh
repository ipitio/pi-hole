...
UpdateSpeedTest() {
    ...
    tmux send-keys -t pimod "curl -sSLN https://github.com/ipitio/pihole-speedtest/raw/ipitio/mod.sh | sudo bash -s -- up ${args[2]}"
}
...
UninstallSpeedTest() {
    ...
    tmux kill-session -t pimod &> /dev/null
    tmux new-session -d -s pimod 
    tmux send-keys -t pimod "curl -sSLN https://github.com/ipitio/pihole-speedtest/raw/ipitio/mod.sh | sudo bash -s -- un"
}
